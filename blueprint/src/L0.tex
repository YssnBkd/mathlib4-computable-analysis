...
\begin{lemma}
  ...
  \leanok
  \uses{def:cantorPair}
  \texttt{cantorPair} is \texttt{Computable}$_2$. The proof composes \texttt{Primrec}$_2$ projections, addition, multiplication, and quotient-by-two via \texttt{Primrec.nat\_mul.comp} / \texttt{Primrec.nat\_div2}, then lifts via \texttt{.to\_comp}.
\end{lemma}
\begin{proof}\leanok
  Formalized in \texttt{ComputableAnalysis/L0/Bridge.lean}.
\end{proof}
...
\end{lemma}
\begin{proof}\leanok
  Formalized in \texttt{ComputableAnalysis/L0/PropB.lean}.
\end{proof}
